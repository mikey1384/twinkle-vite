...
export default function CollaborationPanel({
  ...
  collaborationEventRef.current = (payload: any) => {
    if (Number(payload?.buildId || 0) !== Number(rootBuildId || 0)) return;
    const isMyLeave =
      payload?.inviteStatus === 'left' &&
      Number(payload?.invite?.userId || 0) === Number(userId || 0);
    if (isMyLeave && !isRootOwner) {
      applyContributionInviteStatus({
        invite: payload?.invite,
        inviteId: Number(payload?.invite?.id || payload?.inviteId || 0),
        status: 'left',
        eventTimeMs: Number(payload?.eventTimeMs || Date.now())
      });
      navigate('/build', {
        state: { buildTeamLeaveMessage: 'You left the team.' }
      });
      return;
    }
    ...
    if (
      isRootOwner &&
      isContributionFork &&
      transferredBranchIds.includes(Number(build.id || 0))
    ) {
      void onBuildReload?.();
      return;
    }
  ...
  useEffect(() => {
    function handleCollaborationUpdated(payload: any) {
      collaborationEventRef.current(payload);
    }
    socket.on('build_collaboration_updated', handleCollaborationUpdated);
    return () => {
      socket.off('build_collaboration_updated', handleCollaborationUpdated);
    };
  }, []);
