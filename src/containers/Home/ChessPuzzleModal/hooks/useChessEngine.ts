...
export function useChessEngine() {
  ...
  const pendingRequests = useRef<Map<number, (result: EngineResult) => void>>(new Map());
  ...
  const initializeEngine = useCallback(() => {
    try {
      ...
      workerRef.current.onmessage = (e) => {
        ...
        } else if (type === 'result' && requestId && pendingRequests.current.has(requestId)) {
