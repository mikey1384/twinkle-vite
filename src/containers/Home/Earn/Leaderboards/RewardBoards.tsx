...
import { useEarnStandings } from './useEarnStandings';
...
export default function RewardBoards({ kind }: { kind: 'apps' | 'creators' }) {
  ...
  const title =
    kind === 'apps' ? 'Earned from apps' : 'Whose apps paid the most';
  const standings = useEarnStandings(hub?.standings, loading, kind === 'apps');
  const rows =
    kind === 'apps' ? standings.entries : hub?.creators.entries || [];
      ...
      >
        ...
        {loading && !hub ? (
          ...
          <>
            ...
            <ol className={listClass}>
              ...
              {me && !meVisible && (
                <>
                  ...
                  <Row
                    row={{ ...me, username: me.username || 'You' }}
                    kind="apps"
                    you
                  />
                </>
            ...
            {kind === 'apps' && standings.nextCursor && (
              <div style={{ marginTop: '1.5rem' }}>
                {standings.error && (
                  ...
                  >
                    Could not load more standings. Please try again.
                  </p>
                )}
                <LoadMoreButton
                  onClick={standings.loadMore}
                  loading={standings.loadingMore}
                  disabled={loading || standings.loadingMore}
                  label={standings.error ? 'Try again' : 'Load more'}
                />
              </div>
            )}
          </>
