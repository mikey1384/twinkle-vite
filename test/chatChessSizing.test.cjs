...
function game(tablet) {
  return compile(
    'src/containers/Chat/Chess/Game/BoardFrame.tsx',
    {
...
test('all chess sizes reserve the same CSS dimension that the actual board consumes', () => {
  for (const tablet of [false, true]) {
    ...
    for (const size of ['regular', 'compact', 'inline']) {
      ...
      assert.equal(
        (tree.props.className.match(/--chat-chess-board-size:/g) || []).length,
        2
      );
      if (size === 'compact') {
        ...
        assert.match(
          tree.props.className,
          /--chat-chess-board-size: min\(90vw, 14rem\)/
        );
      }
    }
  }
  ...
  assert.match(
    board,
    /grid-template-rows: var\(--chat-chess-board-size\) 2.5rem/
  );
  assert.doesNotMatch(
    board,
    /const (mobileBoardWidth|boardWidth|defaultBoardWidth)/
  );
